#!/usr/bin/env python3
"""
folderbuilder_v6.py — robust refactor of "folderbuilder v5.py"

Goals implemented (10 improvements):
1) Centralized config + sane defaults (config.ini next to script/EXE). 
2) PyInstaller-friendly resource_path() and bundled-data handling.
3) No destructive global deletes; all ops are contained and explicit.
4) Work in a temp build directory; move final result at the end.
5) Copy shared assets (never move) to avoid breaking subsequent runs.
6) Safer CSV discovery (most recent appsheet*.csv) with age/extension checks.
7) CSV header validation with configurable field names.
8) Absolute paths for all file ops; explicit cwd when launching child EXE.
9) Verbose logging + --dry-run flag; log file next to script/EXE.
10) Guard rails: destructive ops require --force (or config AllowDelete=yes).

Usage examples:
  python folderbuilder_v6.py --dry-run
  folderbuilder_v6.exe --csv "C:\\Users\\John\\Downloads\\appsheet_2025-10-26.csv" --force

This script expects the template ZIP to be named 'folderstructure.zip' and to contain the
folder "video folder template". By default the EXE/script directory acts as BASE_PATH.
"""
...
# -----------------------------
# Logging (simple, no external deps)
# -----------------------------
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    # File handler (rotating ~1MB, keep 3)
...
# -----------------------------
# Config (ini without external libs)
# -----------------------------

def load_config(cfg_path: Path) -> Dict[str, Dict[str, str]]:
    """Minimal ini reader: [section] key=value; ignores comments/blank lines."""
...
def read_csv_rows(csv_path: Path, field_map: Dict[str, str], logger: logging.Logger) -> List[Dict[str, str]]:
    """Read CSV with required headers. field_map maps logical names -> actual CSV headers.
       Required logical names: project, chapter, story, sequence
    """
...
def build_project(args, logger: logging.Logger) -> None:
    ...
    if not template_zip.is_absolute():
        # try bundled resource first, then relative to base/script
    ...
    if args.csv:
        ...
        csv_file = most_recent_csv(csv_dir, pattern_prefix=cfg_csv.get('PATTERN_PREFIX', 'appsheet'),
    ...
    # Expect a folder named "video folder template" inside the zip
    template_root = next((p for p in extract_dir.iterdir() if p.is_dir() and p.name.lower().startswith('video folder template')), None)
    if not template_root:
        raise FileNotFoundError('Could not locate "video folder template" inside the ZIP.')

    # 4) Prepare destination in build dir first
    dest_in_build = build_root / safe_project
    safe_copytree(template_root, dest_in_build, logger)

    # 5) Ensure static subfolders
    ...
    # 6) Create chapter folders (dedupe by lower-case composite)
    seen_keys = set()
    ...
    # 8) Copy timelinevids.exe if present in template (copy, never move)
    ...
    # 9) Move finished build into BASE_PATH (guard rails for destructive ops)
    ...
    if final_dest.exists():
        ...
        if not allow:
            raise FileExistsError(f'Final destination already exists: {final_dest}. Use --force to overwrite.')
        # safe overwrite: remove existing dir
    ...
    # 10) Optionally launch timelinevids.exe
    ...
    # 11) Cleanup extract dir (always safe to remove our own temp)
    if not args.dry_run:
...
def sanitize_name(name: str) -> str:
    # Allow letters, digits, space, dash, underscore, dot; strip others
